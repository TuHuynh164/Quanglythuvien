...
//đêm sách
    function tong_sach($a){
        $sql = new database();
        $sql->select("SELECT COUNT(sach.Id) FROM sach INNER JOIN loai_sach ls ON sach.Ma_ten_sach = ls.Id WHERE ls.Ten_sach = '".$a."'");
        $sl=$sql->fetch();
        return $sl["COUNT(sach.Id)"];
    }
    function tong_sach_trong_kho($a){
        $sql = new database();
        $sql->select("SELECT COUNT(sach.Id) FROM sach INNER JOIN loai_sach ls ON sach.Ma_ten_sach = ls.Id WHERE ls.Ten_sach = '".$a."' AND sach.Hien_co = 1");
        $sl1=$sql->fetch();
